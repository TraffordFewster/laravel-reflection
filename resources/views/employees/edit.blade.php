        ...
        <div class='container'>
            ...
            @csrf
                <div class='col-12 col-md-6'>
                    ...
                    value="{{old('first_name') ? old('first_name') : $employee->first_name }}" required>
                </div>
                <div class='col-12 col-md-6'>
                    <label for="last_name">Last Name</label>
                    <input name='last_name' type="text" class="form-control mb-4" id="last_name" placeholder="Doe" 
                    value="{{(old('last_name') ? old('last_name') : $employee->last_name)}}" required>
                </div>
                <div class='col-12'>
                ...
                <div class="col-12">
                    <select class="custom-select w-100 mb-4" aria-label="Select a company" id='company_id' name='company_id'>
                        ...
                        @foreach ($companies as $comp)
                            <option 
                                @if (old('company_id'))
                                    @if (old('company_id') == $comp->id)
                                        selected
                                    @endif
                                @else
                                    @if ($employee->company && $employee->company->id == $comp->id)
                                        selected
                                    @endif
                                @endif
